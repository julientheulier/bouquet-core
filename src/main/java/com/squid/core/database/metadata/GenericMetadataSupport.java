 ...
 *
 */
public class GenericMetadataSupport implements VendorMetadataSupport {
	...
	public List<ColumnData> getColumns(Connection conn, String catalog, String schemaPattern, String tableNamePattern, String columnNamePattern) throws SQLException {
		...
		while (res.next()) {
            ...
            loadColumnData(res,data);
            normalizeColumnData(data);// fixed T58 rewriting error
            datas.add(data);
        }
        return datas;
    }
	...
	public List<Schema> getSchemas(DatabaseFactory df, Connection conn) throws ExecutionException {
		...
		try {
			...
			int cnt = 0;
			 while (res.next()) {
				 cnt++;
				 String name = res.getString(getColumnDef(MetadataConst.TABLE_SCHEMA));
				 //
			...
			e.printStackTrace();
			throw new ExecutionException("Not able to get schemas",e);
		}
	}
	...
	public List<Schema> getCatalogs(DatabaseFactory df, Connection conn) throws ExecutionException {
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new ExecutionException("failed to get catalogs: "+e.getMessage(),e);
		}
	}
	...
	public List<Table> getTables(DatabaseFactory df, Connection conn, String catalog, String name, String tableName) throws ExecutionException {
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new ExecutionException("Not able to get tables",e);
		}
	}

	@Override
	public ResultSet getImportedKeys(Connection conn, String catalog, String name, String tableName) throws ExecutionException {
		try {
			return conn.getMetaData().getImportedKeys(catalog, name, tableName);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new ExecutionException("Not able to import keys",e);
		}
	}

    ...
    default:
      ...
      ;
        ...
        if (terms==null) {
            ...
            {if (true) return createQuery(scope, expression, terms);}
          }
    throw new Error("Missing return statement in function");
  }

  final public List<QueryTerm> query_terms(ExpressionScope scope) throws ParseException {
    Token operator;
    ExpressionAST expression;
    ...
    default:
      ...
      ;
         ...
         else
        {
            ...
            {if (true) return sub_query;}
        }
    throw new Error("Missing return statement in function");
  }

  final public Token query_operator(ExpressionScope scope) throws ParseException {
    Token operator;
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case FILTER:
      operator = jj_consume_token(FILTER);
                          {if (true) return operator;}
      break;
    case FACET:
      operator = jj_consume_token(FACET);
                         {if (true) return operator;}
      break;
    case METRIC:
      operator = jj_consume_token(METRIC);
                          {if (true) return operator;}
      break;
    default:
      jj_la1[2] = jj_gen;

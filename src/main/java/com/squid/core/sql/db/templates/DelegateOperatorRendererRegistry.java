...
public class DelegateOperatorRendererRegistry {
	
	private HashMap<String, OperatorRenderer> delegateRenderer = new HashMap<String, OperatorRenderer>();
	 ...
	 * @param renderer
	 */
	public void registerOperatorRender(String extendedID, OperatorRenderer renderer) {
		delegateRenderer.put(extendedID, renderer);
	}
	
	public String render(SQLSkin skin, OperatorPiece piece, OperatorDefinition opDef, String[] args) throws RenderingException {
	    // render the default case
		OperatorRenderer renderer = delegateRenderer.get(opDef.getExtendedID());
		if (renderer!=null) {
			return renderer.prettyPrint(skin, piece, opDef, args);
		} else {

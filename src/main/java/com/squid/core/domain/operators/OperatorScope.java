...
implements IntrinsicOperators
{
	...
	private UndefinedOperatorDefinition undefinedOperatorDef =
		new UndefinedOperatorDefinition("UNDEFINED",UNDEFINED_ID);
	...
	private void initIntrinsicOperators() {
		...
		for (String op : ops) {
			registerSafe(new ArithmeticOperatorDefintion(op,"com.squid.domain.opertor.extension."+op,OperatorDefinition.PREFIX_POSITION,op,IDomain.CONTINUOUS, OperatorDefinition.MATHS_TYPE, ExtendedType.FLOAT));
		}
		// -----------------------------------------------------------------------------
		final String[] opstrigo = {"COS","SIN","TAN","ACOS","ASIN","ATAN"};
		for (String op : opstrigo) {
			registerSafe(new ArithmeticOperatorDefintion(op,"com.squid.domain.opertor.extension."+op,OperatorDefinition.PREFIX_POSITION,op,IDomain.CONTINUOUS, OperatorDefinition.TRIGO_TYPE, ExtendedType.FLOAT));
		}
		// -----------------------------------------------------------------------------
	 ...
	 * @throws ScopeException if not exception
	 */
	...
	public OperatorDefinition lookupByName(String name) throws ScopeException {
		OperatorDefinition def = (OperatorDefinition)m_lookupByName.get(name);
		if (def==null) {
	...
	public void registerLegacy(OperatorDefinition def, int ID) throws OperatorScopeException {
		if (m_lookupByName.get(def.getName())==null) m_lookupByName.put(def.getName(),def);
		//
		// manage legacy ID
		int pos = ID-IntrinsicOperators.START_ID;
	...
	/**
	 * @param type is either ALGEBRAIC_TYPE or AGGREGATE_TYPE
	 * @return
	 */
